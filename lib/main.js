  ...
  provideLinter() {
    return {
      ...
      lint: async (textEditor) => {
        ...
        const dir = require('path').dirname(file) + require('path').sep;
        const projectPath = atom.project.relativizePath(dir)[0] == null ? '' : atom.project.relativizePath(dir)[0] + require('path').sep;
        let chartPath;
        ...
        if (lstatSync(`${dir}Chart.yaml`).isFile())
          chartPath = dir;
