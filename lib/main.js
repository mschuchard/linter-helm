  ...
  provideLinter() {
    return {
      ...
      lint: async (textEditor) => {
        ...
        // bail out if the current dir does not contain a Chart.yaml (metadata) since that is the reliable indicator of a chart
        if (!(lstatSync(`${dir}${require('path').sep}Chart.yaml`).isFile())) return [];
        ...
        return helpers.exec(atom.config.get('linter-helm.helmExecutablePath'), args, { ignoreExitCode: true, throwOnStderr: false }).then(output => {
          ...
          output.split(/\r?\n/).forEach((line) => {
            ...
            const lintMatches = /\[(ERROR|WARNING|INFO)\]\s(.*\.yaml):?(\d+)?:?(\d+)?:\s(.*)/.exec(line);
            ...
            if (lintMatches != null) {
              ...
              toReturn.push({
                ...
                location: {
                  file: dir + require('path').sep + lintMatches[2],
